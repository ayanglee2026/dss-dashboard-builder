...
| 方式 | 适用 | 实现 |
|------|------|------|
| 站内徽标 | 低优先级 | header 红点 + 计数，点开跳异常看板 |
| 待办清单 | 中优先级 | 异常进待办分组，点开有策略（本文第 5 节） |
| 邮件 / 企微·钉钉 webhook | 高优先级 | 走 CloudSync 调后端推送（见 `cloud-sync-adapter.md`） |

分级原则：低→站内，高→推送。本地演示版通常只做站内徽标 + 待办，邮件推送留待部署阶段接后端。

## 7. 规则 enabled 字段的坑（真实踩坑）

规则对象漏了 `enabled` 字段时，`if (!rule.enabled)` 会把 `undefined` 当 false，**所有规则被跳过、异常永远 0**（经营日报案例踩过）。判断要写成「默认启用，仅显式 0 才禁用」：

```javascript
// ❌ if (!rule.enabled || rule.enabled === 0) return;
// ✅ 默认启用，仅显式 0 才禁用
if (rule.enabled === 0 || rule.enabled === '0' || rule.enabled === false) return;
```
...
- **待办清单**（哪些实体命中规则）——规则算出来的，**不用存**，每次重新算即可
- **待办处理状态**（标记完成/未完成）——用户操作产生的，**必须持久化**，否则刷新就丢

处理状态持久化方案：
- 纯前端本地/单人：localStorage 存 `{ [实体id]: 'done' }`，刷新不丢、各浏览器独立
- 多人共享：后端存表，跨设备一致

真正要存的是「规则配置 + 处理状态」，不是「待办结果」。选「多人共享 + 待办」却不做后端持久化，「标记完成」就是玩不起来的半成品。
