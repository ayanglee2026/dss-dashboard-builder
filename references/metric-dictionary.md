...
6. **直接字段 vs 派生字段要分清**：Excel 里已存在的字段，先确认是「直接字段」（业务手填）还是「可派生」（公式算）。真实案例：经营日报的「净新签」是业务手填字段，和派生公式（换租签约+总新签−总退租−换租退租）有 141/360 处差异。手填字段直接用，公式仅作旧表兜底——别急着套公式把直接字段当成派生指标
