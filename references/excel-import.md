...
> ⚠️ **父级合计/上报字段必须排除出 SUM_FIELDS**：明细表里常混入「门店合计 / 上报总数」这类父级合计列，把它们塞进 SUM_FIELDS 无脑求和会翻倍（每个明细行都累加一次合计值）。这类字段单独存到 `PARENT_TOTAL_FIELDS`，交叉校验时按父级维度取一次值，与分项之和对比（见第 4 节）。
...
- 空行跳过（整行无有效数据）
- **格式修正也记 warning**，不只是转 Number 失败——用户必须知道数据被改过。格式修正类型清单：去千分位逗号（`12,500`→`12500`）、去全角/半角空格（`1 200`→`1200`）、去换行/制表符、日期格式归一化（`2026/7/1`→`2026-07-01`）
